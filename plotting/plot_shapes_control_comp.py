...
def main(info):
    ...
    rootfile_2018 = rootfile_parser.Rootfile_parser(args.input.replace("2022", "2018").replace("DYWLO", "DYWNLO"), variable)
    ...
    for index, category in enumerate(plot_names):
        ...
        plot.add_hist(
            rootfile_2018.get(channel, "data", category, shape_type=stype, new_name_tag="_2018"),
            category+"_2018",
        )
    plot_names.append("data_2018")
    ...
    for data_name in plot_names:
        ...
        if args.blinded:
            ...
            if "2018" in data_name:
                plot.subplot(0).setGraphStyle(data_name, "e0", markercolor=ROOT.kRed, markershape=22, linecolor=ROOT.kRed)
            else:
        ...
        if "2018" in data_name:
            plot.subplot(2).setGraphStyle(data_name, "e0", markercolor=ROOT.kRed, markershape=22, linecolor=ROOT.kRed)
        else:
    ...
    for i in range(2):
        ...
        for index in draw_list:
            ...
            if "2018" in index:
                data_label_name = "Data 2018"
            else:
                data_label_name = "Data 2022"
