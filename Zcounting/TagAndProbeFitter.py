...
class TagAndProbeFitter:
    ...
    def set_fit_var(self, v='x', vMin=55, vMax=125,
        ...
        self._w.var(v).setBins(50000,"cache")
    ...
    def fit(self, outFName, lumi_online, nZ_MC, mcTruth=False, template=True):
        ...
        text.AddText("    - {} \t= {:.3f} #pm {:.3f} (stat.) {}".format(
            "Lumi", lumi_Z_fromMC, lumi_Z_fromMC_err, "pb^{-1}"
        ))
        text.AddText("    - {} \t= {:.3f} #pm {:.3f} (stat.)".format(
            "Lumi/online", lumi_Z_fromMC/lumi_online, lumi_Z_fromMC_err/lumi_online
        ))
        ...
        return lumi_Z_fromMC, lumi_Z_fromMC_err
